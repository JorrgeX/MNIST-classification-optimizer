...
kmax = 6e2;
tol = 1e-3;
% params for stochastic
len = length(Xtrain);
q_loss = @(I,w,lam)qloss(I,Xtrain,label,w,lam);
grad_qloss = @(I,w,lam)qlossgrad(I,Xtrain,label,w,lam);

% choose algorithm
% direction = 1: LevenbergMarquardt
% direction = 2: Stochastic gradient descent
% direction = 3: Stochastic Nesterov
% direction = 4: Stochastic Adam
direction = 1;

switch direction
    case 1 % LevenbergMarquardt
        [w,f,gnorm] = LevenbergMarquardt(r_and_J,w,kmax,tol);

    case 2 % Stochastic gradient descent
        [w,f,gnorm] = SGD(q_loss,grad_qloss,w,len,kmax,tol);

    case 3 % Stochastic Nesterov
        [w,f,gnorm] = StochasticNesterov(q_loss,grad_qloss,w,len,kmax,tol);
    
    case 4 % Stochastic Adam
        [w,f,gnorm] = StochasticAdam(q_loss,grad_qloss,w,len,kmax,tol);
...
function q = myquad(Xtrain,label,I,w)
X = Xtrain(I,:);
d = size(X,2);
d2 = d^2;
y = label(I);
...
f = sum(log(1 + exp(-myquad(Xtrain,label,I,w))))/length(I) + 0.5*lam*w'*w;
end

function g = qlossgrad(I,Xtrain,label,w,lam)
aux = exp(-myquad(Xtrain,label,I,w));
a = -aux./(1+aux);
X = Xtrain(I,:);
d = size(X,2);
d2 = d^2;
